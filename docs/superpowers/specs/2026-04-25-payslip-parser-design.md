...
|---|---|---|---|---|---|---|---|---|---|---|---|
| Company | Pay Period Begin | Pay Period End | Check Date | Gross Pay | Pre Tax Deductions | Employee Taxes | Post Tax Deductions | Net Pay | 401k Salary | 401k Employer Match | Life@ Choice |
...
Appends a row to `Payslips!A:L` with the 12 fields in column order. Dates serialized as `YYYY-MM-DD` strings.

---

## Updated: `backend/init_sheets.py`

Add Payslips tab initialization: write the 12 column headers to `Payslips!A1:L1` if the sheet exists, or log a warning if not. Pattern matches existing Transactions/Categories tab setup.
